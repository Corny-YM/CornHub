...
const ReactionsButton = ({ totalReactions, onClick }: Props) => {
  ...
  return (
    <Button
      className="px-2 py-1 h-fit flex items-center gap-1 text-xs text-inherit cursor-pointer select-none rounded-full leading-normal hover:bg-zinc-400/50 dark:hover:bg-primary-foreground"
      variant="outline"
      size="sm"
      onClick={onClick}
    >
